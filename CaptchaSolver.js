...
class CaptchaSolver {
  ...
  async _solveCaptcha() {
    ...
    while (currentPosition < slider.width - handle.width / 2) {
      ...
      if (target.difference > difference) {
        ...
        require("fs").writeFile("./start.png", this.startImage, 'base64', function (err) {
          // console.log(err);
        });
        require("fs").writeFile("./" + currentPosition + "currentImage.png", currentImage, 'base64', function (err) {
          // console.log(err);
        });
  ...
  async _watchForCaptchaAdd() {
    try {
      ...
      const waitForCaptchaElements = this.captchaElements.map((el) => this.page.waitForSelector(el, {
        state: 'attached'
      }))
  ...
  async _waitForCaptchaDomRemove({
    verifyElement,
    verifyContainer
  }) {
    return new Promise((resolve, reject) => {
  ...
  _syncOverlayPositionWithPuzzlePiece({
    puzzlePiece,
    puzzlePieceOverlay
  }) {
  ...
  _removeOverlayAndShowPuzzlePiece({
    puzzlePieceOverlay,
    puzzlePiece
  }) {
